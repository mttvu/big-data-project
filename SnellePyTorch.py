...
# LSTM model geinspireerd op tutorial https://stackabuse.com/time-series-prediction-using-lstm-with-pytorch-in-python/
...
class thing:
    def __init__(self, DATA, N_FORECAST=10000, StartingTimeDelta=datetime.timedelta(minutes=10), newTimeDelta=datetime.timedelta(minutes=60), timeCol='time', dataCol='waterDepth', seq=30):
        if timeCol == 'index':
        ...
        print("NAN in data is:", self.data.isnull().values.any())
        ...
        self.train, self.test = self.data[0:len(self.data)-self.N_FORECAST], self.data[-self.N_FORECAST:]
    ...
    def predict(self, model, data=None):
        ...
        if data != None:
            test_inputs = data[-self.train_window:].tolist()
        else:
        ...
        for i in range(fut_pred):
            seq = torch.FloatTensor(test_inputs[-self.train_window:]).to(self.device)
            with torch.no_grad():
                # Changed from model.hidden (fout in tut)
                model.hidden_cell = (torch.zeros(1, 1, model.hidden_layer_size).to(self.device),
                                torch.zeros(1, 1, model.hidden_layer_size).to(self.device))
    ...
    def accCheck(self, model, trainData, testData):
        # fig = plt.figure()
        # ax = fig.add_subplot(111)

        # Plot the Origional and interpolated data
        # ax.plot(trainData.index, trainData['Data'], label="x_train")
        # ax.plot(testData.index, testData['Data'], label="x_test")

        # Normalize the data for the model
        scaler = MinMaxScaler(feature_range=(-1, 1))
        data_normalized = torch.FloatTensor(scaler.fit_transform(np.array(trainData).reshape(-1, 1))).view(-1).to(self.device)

        # seasonal_periods=self.sesper, seasonal='multiplicative'
        prediction = self.predict(model, data_normalized)

        # prediction = ARIMA(self.trainx['Data'], order=(2,1,3), seasonal_order=(0,0,0,30)).fit().forecast(int(len(self.testx)))
        # ax.plot(testData.index, prediction[self.datacol], label="Predicted")

        # plt.legend()
        # plt.show()
        return trainData, testData, prediction
...
if __name__ == '__main__':
    ...
    # Gebruikte meetpunt
    n = 'Amerongen boven'
    meetpunt = meetpunten[meetpunten['name'] == 'Vuren']
    ...
    # waterlevel_df = rws.get_waterlevel(start, end, 'Vuren')
    # # waterdepth_df = rws.get_covadem_waterdepth(start, end, meetpunten['name'][naam], 0.5)
    # # bedlevel_df = calculate_bedlevel(waterlevel_df, waterdepth_df)
    #             # print("First:", len(waterdepth_df))
    # # trainx, testx = train_test_split(waterlevel_df, test_size=0.2, shuffle=False)
    # # a = [waterlevel_df, waterdepth_df, bedlevel_df]
    
    #                 # print("Second:", len(predicting.data))
    # model = predicting.train(epochs=250)
    # predicting.saveModel("machine_learning//MODELS//waterLevel//" + str('Vuren') + "_waterLevel_V2.pt", model)
    #             # print(predicting.predict(model))
    ...
    for naam in tqdm(range(len(meetpunten['name'])), desc='All points'):
        # try:
        waterlevel_df = rws.get_waterlevel(start, end, meetpunten['name'][naam])
        predicting = thing(DATA=waterlevel_df, N_FORECAST=(30), StartingTimeDelta=datetime.timedelta(minutes=10), newTimeDelta=datetime.timedelta(days=1), timeCol='time', dataCol='NUMERIEKEWAARDE', seq=30)
        model = LSTM(device='cuda:0')
        state = torch.load("machine_learning//MODELS//WaterLevel//" + str(meetpunten['name'][naam]) + "_waterLevel_V2.pt", map_location='cuda:0')
        model.load_state_dict(state)
        # predicting.predict(model)
        # predicting.plot()
        print(predicting.test)
        print(predicting.train)
        predicting.accCheck(model, testData=predicting.test, trainData=predicting.train)
        # print(predicting.predict(predicting.loadPickleModel("machine_learning//MODELS//WaterDepth//" + str(meetpunten['name'][naam]) + "_waterDiepte_V1.pkl")))
        # predicting.plot()
