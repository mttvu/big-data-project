...
if __name__ == '__main__':
    ...
    for naam in tqdm(range(len(meetpunten['name'])), desc='All points'):
        try:
            waterlevel_df = rws.get_covadem_waterdepth(start, end,  meetpunten['name'][naam], 0.5)
            predicting = thing(DATA=waterlevel_df, N_FORECAST=(300), StartingTimeDelta=datetime.timedelta(minutes=10), newTimeDelta=datetime.timedelta(days=1), timeCol='time', dataCol='y')
            model = predicting.train(epochs=300)
            predicting.saveModel("machine_learning//MODELS//WaterDepth//" + str(meetpunten['name'][naam]) + "_waterDiepte_V1.pkl", model)
        except Exception as e: 
            print(e)
            continue

    predicting.predict(predicting.loadPickleModel("machine_learning//MODELS//waterLevel//" + str(waterlevel_df['MEETPUNT_IDENTIFICATIE'][0]) + "_waterDiepte_V1.pkl"))
    predicting.plot()
