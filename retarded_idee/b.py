    ...
    def train(self, epochs=150, model=None):
        ...
        for i in tqdm(range(epochs)):
            for seq, labels in self.train_inout_seq:
                optimizer.zero_grad()
                model.hidden_cell = (torch.rand(1, 1, model.hidden_layer_size).to(self.device),
                                        torch.rand(1, 1, model.hidden_layer_size).to(self.device))
...
if __name__ == '__main__':
    ...
    for naam in tqdm(range(len(meetpunten['name'])), desc='All points'):
        try:
            ...
            predicting = thing(DATA=bedlevel_df, N_FORECAST=(300), StartingTimeDelta=datetime.timedelta(minutes=10), newTimeDelta=datetime.timedelta(days=1), timeCol='index', dataCol='bedlevel')
            # model = predicting.train(epochs=300)
            # predicting.saveModel("machine_learning//MODELS//BodelLigging//" + str(meetpunten['name'][naam]) + "_bodemligging_V1.pkl", model)
            print(predicting.predict(predicting.loadPickleModel("machine_learning//MODELS//BodelLigging//" + str(meetpunten['name'][naam]) + "_bodemligging_V1.pkl")))
            predicting.plot()
